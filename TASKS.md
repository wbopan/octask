...
- [x] 重命名项目为更贴切的名字 #rename-project
    当前项目名 "task-management-plugin" 过于通用，需要一个更具辨识度、能体现 phase-gate 看板 + Claude Code 集成特色的名字。涉及 repo 目录名、package.json、plugin.json、CLAUDE.md、README 等处的更新。
...
- [-] 桌面应用 PWA 支持 #pwa-support
    为 dashboard 添加 PWA manifest 和 service worker，使其可通过浏览器"安装为应用"作为独立桌面窗口运行，脱离浏览器 tab。
    AC: 用户可从 Chrome/Edge 将 dashboard 安装为桌面应用，独立窗口运行且离线时显示上次缓存的状态。
...
- [/] Skill 规范补充：backlog 用法与 phase 创建准则 #skill-backlog-phase-guidance
    在 task-management skill 中补充实践指南：(1) backlog `[-]` 适用于"好主意但时机未到"的任务，不是垃圾桶；首先确认一下，现在的 SPEAR 里面这些东西都没怎么描述
    AC: Skill 文档包含 backlog 使用场景说明和 phase 创建门槛准则；AI 遵循规范时不会为单个小任务创建新 phase。
